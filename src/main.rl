...
(def create-graphics-pipeline proc [surface] seq
  [let! ;; shaders 
        shaders (list.list (load-shader "resources/shaders/vert.spv") (load-shader "resources/shaders/frag.spv"))

        vertex-binding-descriptions (list.list
          (struct hedron.BindingDescription
            [.binding 0]
            [.stride narrow (size-of Vertex) U32]
            [.input-rate :vertex]))

        vertex-attribute-descriptions (list.list
...
(def draw-frame proc [(acquire SyncAcquire) (submit (list.List SyncSubmit))
...
(def new-winsize proc [(messages (list.List window.Message))] seq
...
(def main proc [] seq
  [let! ;; windowing !
        ...
        indices (is (list.list 0 1 2 2 3 0) (list.List U16))
